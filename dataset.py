...
class Dataset:
    ...
    def test_points(self):
        ...
        return x_test.to(self.device), triangles


if __name__ == "__main__":
    dataset = Dataset(domain="flower")
    test_points, triangle = dataset.test_points()
    u = torch.sin(6 * test_points[:, 0]) * torch.sin(6 * test_points[:, 1])
    ...
    if type(triangle) is int:
        plt.tricontourf(test_points[:, 0], test_points[:, 1], u, 100, cmap="jet")
    else:
        plt.tricontourf(test_points[:, 0], test_points[:, 1], triangle, u, 100, cmap="jet")
    plt.show()
